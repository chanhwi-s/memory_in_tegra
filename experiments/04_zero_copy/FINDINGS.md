...
Generated 2026-07-24T12:25:13Z from `experiments/04_zero_copy/results/phase4_results.csv`. All numbers below are computed directly from that CSV (and `results/l2_profile.csv` if present) by `scripts/derive_findings.py` -- re-run it (do not hand-edit) if either CSV changes. This run **supersedes** the first Phase 4 run per `prompts/04_zero_copy_v2.md`.

## Headline

Across the widened asymmetric K1 sweep (1.50MB -> 24.62MB), zero copy helps (memory-bound) in the 1.50MB-2.62MB range, peaking at asym_00 (l1 regime): +15.8% at reuse N=1, crossing over around reuse N=>32. Beyond that (2.00MB-24.62MB), the workload is DRAM/compute-limited enough that bypassing cache costs throughput instead (zero copy loses).
...
|---|---|---|---|---|---|
| asym_2 | shared | 1 | 122.8 | 134.6 | pass |
| asym_2 | shared | 2 | 146.3 | 154.1 | pass |
| asym_2 | shared | 4 | 161.8 | 166.2 | pass |
| asym_2 | shared | 8 | 175.0 | 176.0 | pass |
| asym_2 | shared | 16 | 183.2 | 182.0 | FAIL |
| asym_2 | shared | 32 | 187.8 | 185.7 | FAIL |
...
| sym_1 | shared | 1 | 84.3 | 98.5 | pass |
| sym_1 | shared | 2 | 128.1 | 125.7 | FAIL |
| sym_1 | shared | 4 | 171.2 | 145.2 | FAIL |
| sym_1 | shared | 8 | 198.0 | 179.5 | FAIL |
| sym_1 | shared | 16 | 217.8 | 198.9 | FAIL |
| sym_1 | shared | 32 | 231.0 | 210.1 | FAIL |
...
|---|---|---|---|---|---|---|---|
| asym_00 | l1 | 1.50MB | +15.8% | none in 1..32 | memory-bound | False | NO -- l2_hit_rate_zc not profiled yet (run scripts/profile_l2.sh) |
| asym_2 | dram | 2.00MB | -10.1% | 8 | compute-bound | False | NO -- l2_hit_rate_zc not profiled yet (run scripts/profile_l2.sh) |
| asym_02 | dram | 2.62MB | +3.1% | 2 | memory-bound | False | NO -- l2_hit_rate_zc not profiled yet (run scripts/profile_l2.sh) |
| asym_03 | dram | 4.59MB | +0.5% | 2 | compute-bound | False | NO -- l2_hit_rate_zc not profiled yet (run scripts/profile_l2.sh) |
| asym_04 | dram | 8.04MB | +0.4% | 2 | compute-bound | False | NO -- l2_hit_rate_zc not profiled yet (run scripts/profile_l2.sh) |
| asym_05 | dram | 14.07MB | +0.0% | 16 | compute-bound | False | NO -- l2_hit_rate_zc not profiled yet (run scripts/profile_l2.sh) |
| asym_06 | dram | 24.62MB | +0.0% | 8 | compute-bound | False | NO -- l2_hit_rate_zc not profiled yet (run scripts/profile_l2.sh) |
...
|---|---|---|---|---|
| sym_0 | 768KB | +16.4% | 4 | memory-bound |
| sym_1 | 896KB | +22.0% | 32 | memory-bound |
| sym_ctx_large | 24.62MB | -0.0% | none in 1..32 | dram-bound-throughout |
| sym_ctx_small | 1.50MB | +1.1% | 8 | compute-bound |
