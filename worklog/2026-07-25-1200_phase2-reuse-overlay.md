...
is a macOS dev machine with no CUDA toolchain (`nvcc` not found). Follow-up in the same session,
per user request ("기존처럼 run.sh하면 추가된거 다 돌아가게 해줘" — "make it so running run.sh as
before also runs the added stuff"): wired the reuse overlay into `scripts/run.sh` itself, so the
single existing entry point now produces the reuse overlay outputs too, instead of requiring the
separate `scripts/run_reuse_overlay.sh` invocation.
...
- `scripts/run.sh` (follow-up, same session): added the reuse-overlay sequence (binary with
  `--reuse-out`, `plot_reuse.py`, `append_reuse_findings.py`) as the last steps, placed *after*
  `derive_findings.py` for the same ordering reason documented in `append_reuse_findings.py`'s
  docstring. Reuses the already-built binary, already-generated config, and already-locked clocks
  from earlier in the same script — no duplicated work. Updated the final "Done." echo lines and
  the top-of-file comment to mention the reuse overlay outputs.
- `scripts/run_reuse_overlay.sh` (follow-up): updated the header comment to note that `run.sh`
  now covers this automatically; the script itself is unchanged and remains useful for
  standalone re-runs of just the reuse overlay (e.g. after editing `plot_reuse.py`) without
  re-running the whole main sweep.
- `README.md` (follow-up): updated the "Running" section and the v2 add-on section to reflect
  that `run.sh` is now the single entry point for both the main sweep and the reuse overlay.
...
1. On the Jetson AGX Orin, just run `experiments/02_two_kernel_size/scripts/run.sh` as before —
   it now runs the main sweep *and* the reuse overlay end to end in one invocation (build,
   gen_config, clock lock, main sweep, plot, derive_findings, then the reuse-overlay sweep,
   plot_reuse, append_reuse_findings). Use `scripts/run_reuse_overlay.sh` standalone only if you
   want to re-run just the reuse overlay without redoing the whole main sweep.
...
4. If `derive_findings.py` is ever re-run standalone (outside of `run.sh`) after the reuse
   overlay has already been appended, re-run `scripts/append_reuse_findings.py` afterward — it
   rewrites `FINDINGS.md` from scratch and would otherwise silently drop the "Reuse overlay"
   section. `run.sh` itself already runs them in the safe order.
